...
class _BSTMaxNodeAnimationState extends State<BSTMaxNodeAnimation> {
  BSTNode? root;
  ...
  // Perform maximum node search and update UI step by step
  Future<void> performMaxNodeSearch() async {
    setState(() {});
    await _findMaxNode(root);
  }
  ...
  Future<void> _findMaxNode(BSTNode? node) async {
    ...
    if (node.right != null) {
      await _findMaxNode(node.right); // Continue searching the right subtree
    }
  }
...
void main() => runApp(MaterialApp(home: Scaffold(body: BSTMaxNodeAnimation())));
